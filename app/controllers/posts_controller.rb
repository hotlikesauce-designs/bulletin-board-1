class PostsController < ApplicationController
  ...
  def create
    params_board_id = params.fetch("query_board_id")
    ...
    the_post.board_id = params_board_id

    if the_post.valid?
      the_post.save
      redirect_to("/boards/#{params_board_id}", { :notice => "Post created successfully." })
    else
      redirect_to("/boards/#{params_board_id}", { :alert => the_post.errors.full_messages.to_sentence })
    end
  end
